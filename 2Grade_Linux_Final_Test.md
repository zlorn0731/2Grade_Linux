...
- 컴파일과 인터프리터 차이점
  - 컴파일은 전체 번역 후 실행 파일 생성, 인터프리터는 실행하면서 한 줄씩 번역함
...
- 시스템 호출과 라이브러리 함수의 비교
  ...
  - 한줄 : 시스템 호출은 운영체제의 서비스를 직접 요청하는 함수이고, 라이브러리 함수는 프로그램 개발을 위해 제공되는 미리 작성된 함수
- make 파일 목적 : 다중 소스 파일을 하나로 묶어서 사용하기 위함

## 10장
- fork() : 프로세스 생성
  - 프로세스 : 실행 중인 프로그램
  - 현재 프로세스 복사해서 새 프로세스 하나 더 생성하는 것
 
## 11장
- 소켓 : 네트워크 통신을 위한 통로
- TCP vs UDP
  - 특징
    - TCP : 연결 지향, 신뢰성 , 데이터 순서 보장, 데이터 손실 시 재전송
    - UDP : 비연결형, 신뢰성 보장 안함, 빠름, 순서 보장 안함
- 소켓 연결 과정
```
 클라이언트               서버
  [socket]             [socket]
...
- socket → bind → listen → accept
  - socket() : 소켓 생성
  - bind() : IP 주소와 포트 번호 할당, 이름 부여
  - listen() : 접속 요청 대기
  - accept() : 접속 요청 수락 및 연결 생성
